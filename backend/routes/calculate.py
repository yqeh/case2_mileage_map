    ...
    try:
        ...
        for idx, record in enumerate(records):
            try:
                ...
                # 若 Excel 有「起點地址」「終點地址」則直接使用，不 geocode
                origin_from_excel = (record.get("起點地址") or "").strip()
                dest_from_excel = (record.get("終點地址") or "").strip()

                if not origin_name and not origin_from_excel:
                    errors.append(f"第 {idx + 1} 筆資料缺少起點（起點名稱或起點地址）")
                    updated_records.append(record)
                    continue
                if not destination_name and not dest_from_excel:
                    errors.append(f"第 {idx + 1} 筆資料缺少終點（目的地名稱或終點地址）")
                    updated_records.append(record)
                    continue

                # 起點：優先使用 Excel 的起點地址
                if origin_from_excel:
                    origin_address = origin_from_excel
                    logger.info(f"第 {idx + 1} 筆資料起點使用 Excel 地址: {origin_address}")
                elif fixed_origin:
                ...
                # 終點：優先使用 Excel 的終點地址
                if dest_from_excel:
                    destination_address = dest_from_excel
                    logger.info(f"第 {idx + 1} 筆資料終點使用 Excel 地址: {destination_address}")
                else:
                ...
                try:
                    ...
                    if not exists or not screenshot_path:
                        ...
                        screenshot_path = None
                    # 截圖成功後不再套用 annotate（不在地圖上畫 badge/文字），保留整頁 Google Map 畫面
